#!/usr/bin/env bash
# init_merlean — one-command setup for the MerLEAN system (replaces the old src/deploy.sh).
# .claude/ (skills + agents + settings) is tracked in git, so nothing is mirrored: this script
...
#
#   full setup:            bash .claude/skills/init_merlean/setup.sh
#   with a key:            bash .claude/skills/init_merlean/setup.sh --openai-key sk-...
#   skip the slow parts:   bash .claude/skills/init_merlean/setup.sh --skip-mathlib
#
# Non-interactive by design (safe to run from a Claude Code skill). Re-running is idempotent.
set -euo pipefail
...
if resolve_venv_py; then
  sed "s|__VENV_PY__|$VENV_PY|g" "$TPL/mcp.json" > "$REPO_ROOT/.mcp.json"
  ok ".mcp.json generated (lean-lsp MCP server runs from the venv)"
else
...
echo ""
echo "Done. Verify:  $VENV_PY src/cli.py --data <DATA_DIR> status"
