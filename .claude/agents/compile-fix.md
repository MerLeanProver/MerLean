...
tools: Read, Edit, Bash, Grep, Glob, mcp__lean-lsp__lean_diagnostic_messages, mcp__lean-lsp__lean_goal, mcp__lean-lsp__lean_hover_info, mcp__lean-lsp__lean_leansearch, mcp__lean-lsp__lean_loogle
model: opus
---
...
## Path safety in isolated worktrees
Subagents in this harness may run inside `.claude/worktrees/agent-*`. If `LIB_DIR` points outside
your current working directory, **do not edit that outside checkout directly**. Instead:
1. Copy the entire Lean workspace directory `LIB_DIR` into your current worktree (same basename),
   preserving files.
2. Work only on the copied workspace's `FILE` path; any fallback `lake build` runs inside the
   copied `LIB_DIR`.
3. In your final response, include a small unified diff for the target file relative to the copied
   workspace, in addition to the standard status fields.
4. The orchestrator will apply that diff to the real checkout and run the authoritative build.

If the workspace is already inside your current working directory, edit it normally. If neither direct
editing nor copy-and-patch is possible, return `infra_failure`. This prevents "successful" subagent
edits from landing in the wrong tree or being lost with a stale worktree.
...
`.venv/bin/python src/cli.py --data <DATA_DIR> search "<what the statement claims>"` (read-only;
...
3. Repeat until error-free, then a **warning pass** — warnings COUNT (`unused variable`/`simp`,
   ...
   When diagnostics show 0 errors and 0 warnings, sync the graph (step 4) and report `clean`.
...
4. **Sync the graph after a clean build:**
   `.venv/bin/python src/cli.py --data <DATA_DIR> sync-lean <LIB_DIR>` — reconciles this node's
   dependency edges to the real Lean imports. This is the ONE mutating `cli.py` command you may
   run. Skip it (and say so in `graph_sync:`) if the store is locked, or if you worked on an
   isolated copy of the workspace — the orchestrator syncs after applying your patch.
...
fast; (2) separate nodes let the orchestrator prove the pieces **in parallel** (≤ 4 background
subagents at a time) and bank each independently, whereas one mega-file is strictly serial and
...
file: <FILE>
result: <1–3 sentences: what built, or exactly why not; if you worked on a copied isolated
  workspace, say so and include/apply-ready unified diff below>
graph_sync: <done | skipped (store lock) | skipped (isolated copy) | n/a (not clean)>
...
patch: <empty if you edited the real workspace directly; otherwise a unified diff for FILE that the
  orchestrator can apply to the real checkout>
```
