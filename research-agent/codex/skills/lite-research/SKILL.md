...
name: lite-research
description: >-
  MerLean-lite resolves an informal math question quickly with natural-language proofs, Python
  experiments, adversarial debate, and Lean 4 only at the weakest links. It races distinct
  representation-changing routes, develops a graded step ledger, probes testable claims, and
  Lean-certifies residual uncertainty. Use when asked to lite-research, quickly settle, or screen
  a question before full formalization. Output is graded BRONZE, SILVER, or GOLD.
---

# lite-research — MerLean-lite: NL-first, Lean at the weakest links

Resolve `<PLUGIN_ROOT>` from this `SKILL.md` location before using the optional graph runtime.
Use `<PLUGIN_ROOT>/scripts/merlean` for graph calls. The mode still supports its documented
zero-setup path when the runtime is absent.
...
Fleet: at most `min(4, available child slots)` background subagents (three with four total Codex
slots including the main agent); only compile-fix jobs are RAM-gated (auto-research §R rules).
Scouts/skeptics/explores are cheap — prefer breadth there.
