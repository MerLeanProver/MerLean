...
name: plan-graph
description: >-
  Read, edit, and semantically search the Mem0-g formalization plan. Use to inspect or modify
  statements, dependencies, status, and informal notes; synchronize Lean dependencies; find
  statements by meaning before adding one; or recall prior research. Wraps the plan_store CLI
  with embedding recall and LLM reranking.
---

# plan-graph — drive the Mem0-g statement plan

Resolve `<PLUGIN_ROOT>` from this `SKILL.md` location before any command: it is the directory
two levels above the skill folder. The portable CLI wrapper is `<PLUGIN_ROOT>/scripts/merlean`.
...
```
(Set up the venv once with the `init-merlean` skill; deps are pinned in `src/requirements.txt`.)
...
The online Mem0-g backend is the default. When the OpenAI API is unavailable (including quota or
network failures), select the local fallback explicitly for every invocation in that run:
```
PLAN_OFFLINE=1 <PLUGIN_ROOT>/scripts/merlean --data <DATA_DIR> <command> [...]
```
Keep `PLAN_OFFLINE=1` consistent across the run. Offline mode has command parity and persists to
`<DATA_DIR>/.merlean-offline.json` plus the normal materialized JSON views. It can bootstrap from
existing `statements.json`/`progress.json`. It makes no embedding, vector-store, reranker, or
summary API calls: `search` uses deterministic weighted lexical matching and `summarize` writes a
deterministic dependency/status report. The online backend remains unchanged when the variable is
unset.
...
| `levels` | statements grouped by dependency layer (level 0 = no deps; same-level = independent) — the parallel schedule; obey the current Codex child-slot cap |
...
| `summarize` | Narrative summary of the plan → `summary.md` + a searchable `Summary` meta-node (LLM online; deterministic dependency/status report offline; excluded from graph/status) |
...
```
<PLUGIN_ROOT>/scripts/merlean --data <DATA_DIR> \
...
With `PLAN_OFFLINE=1`, the same two-group output is ranked locally by weighted query-token
coverage, token-set Jaccard similarity, and an exact-phrase bonus; ties use stable insertion order
then statement id. For schema compatibility, `embed_score` contains this lexical score and
