...
name: prove
description: >-
  Fill every sorry or admit in an existing Lean file with real proofs using the Mem0-g plan graph
  and compile-fix subagents. Run in the main context when asked to prove sorries, complete
  unfinished declarations, or formalize a stubbed Lean file. Success requires a clean build with
  no residual sorry, admit, or project axiom.
---
...
two levels above the skill folder. Use `<PLUGIN_ROOT>/scripts/merlean` for all plan-store calls.
...
feasibility, axiom policy, or "how should I proceed" decisions; do not ask a question, do
...
`<PLUGIN_ROOT>/scripts/merlean --data <DATA_DIR> status` (the plan-graph CLI).
...
`add` a graph node:
```
<PLUGIN_ROOT>/scripts/merlean --data <DATA_DIR> add --json '{"id":"Lem_<Name>","type":"Lemma","name":"<Name>",
...
### 3. Prove loop — dispatch compile-fix subagents in the background, bank on each completion
Run the proof loop **asynchronously**. Dispatch each `compile-fix` as a **background** subagent
and **never block** waiting for it. Use at most `min(4, available child slots)` workers; with four
total Codex slots including the main agent, this is three. Queue the rest and dispatch as slots free.
While subagents
prove files, the main context (you, the update agent) keeps making progress — scaffolding new
files, decomposing hard nodes (§3b), proving tractable glue, researching Mathlib. Codex subagents
share the workspace: assign each an exclusive file and do not edit a live worker's file. The run
ends when the **main theorem node is `completed`** (§4).
...
- Pick `pending` nodes whose **dependency files are scaffolded** (signatures locked — they need
  ...
  `compile-fix` with `FILE=<node.lean_path>, STATEMENT_ID=<id>, DATA_DIR, LIB_DIR,
  AXIOM_POLICY=strict`. It proves **only its own file**. Dispatch independent ready nodes together,
  up to the available child-slot cap, and never two that could touch the same file; queue the rest.
...
AND lets the loop (§3) prove the pieces **in parallel** (within the available child-slot cap). You
...
- **No duplicate declarations:** after a final `sync-lean`, run
  `<PLUGIN_ROOT>/scripts/merlean --data <DATA_DIR> lint`;
